...
class Program
{
    ...
    static async Task RunFileWatcherAsync(string filePath, SharedTimestamp sharedTs, string? apiKey, CancellationToken ct)
    {
        ...
        try
        {
            ...
            while (!ct.IsCancellationRequested)
            {
                ...
                var safeGameName = string.IsNullOrWhiteSpace(gameName) ? "Unknown" : gameName;

                Console.WriteLine($"Detected new score write at {timestamp.Value:u}. Score={score.Value}, Code={code}, Game={safeGameName}");
                var ok = await SubmitCodeAsync(code, score.Value, safeGameName, apiKey);
                ...
                if (File.Exists(filePath))
                {
                    ...
                    if (writeMs > sharedTs.Read())
                    {
                        ...
                        if (score.HasValue && timestamp.HasValue)
                        {
                            ...
                            Console.WriteLine($"Poller detected new score write at {timestamp.Value:u}. Score={score.Value}, Code={code}, Game={safeGameName}");
                            var ok = await SubmitCodeAsync(code, score.Value, safeGameName, apiKey);
                            Console.WriteLine(ok ? "Code+score submitted!" : "Failed to submit code+score");
    ...
    // Make nullable where you actually return nulls; then coalesce at call sites.
    static async Task<(int? score, DateTimeOffset? timestamp, string? state, string? code, string? gameName)>
